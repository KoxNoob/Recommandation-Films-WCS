...
if vue == 'Accueil' :
    st.image('https://github.com/KoxNoob/Recommandation-Films-WCS/blob/master/Screenshot%20from%202020-05-13%2014-30-59.png?raw=true', width = 700)
    """ Bienvenue chez DataFlix !"""
...
if vue == 'Recommandations':
    ...
    st.markdown("<h3 style='text-align: center; color: grey; size = 0'>Maintenant que vous êtes bien installés, il n'y a plus qu'à rentrer \
         vos préférences dans la barre de naviguation à gauche, et laisser nous faire !</h3>", unsafe_allow_html=True)
    sex = st.sidebar.selectbox('Votre sexe ?',
                ('Femme', 'Homme', 'Peu importe'))

    age = st.sidebar.selectbox('Dans quelle tranche d\'âge êtes vous ?',
                ('18-29 ans', '30-44 ans', '+ de 45 ans'))

    genre = st.sidebar.selectbox('Quel genre de films aimez-vous ?',
                ('Biography','Drama','Adventure','History','Crime','Western','Fantasy','Comedy','Horror','Family','Action'\
                ,'Romance','Mystery','Animation','Sci-Fi','Musical','Thriller','Music','Film-Noir','War','Sport','Adult'\
                ,'Documentary')) 

    note = st.sidebar.slider('Vous aimeriez voir les films ayant quelle note de moyenne ?', 0, 10, 1)

    values = st.sidebar.slider('Sélectionner un intervalle d\'année', 1906, 2019, (1980, 2010))
    annee_min = values[0]
    annee_max = values[1]
    ...
    def list_parser(liste):
        ...
        for x in range(1, len(url_list)+1):
            if url_list[x-1] != "":
                ...
                globals()['poster%s' % x] = "https://raw.githubusercontent.com/KoxNoob/Recommandation-Films-WCS/master/papiers-peints-vecteur-message-de-pixel-game-over.jpg.jpg"
        return url_list, poster1, poster2, poster3, poster4, poster5
    
    attente = np.random.choice(['Tiens, et si tu allais te chercher une bière ? Ou du popcorn ? Non ? Tu as raison, tout est prêt, voici notre sélection ! ',\
    ...
    display_min = 0
    display_max = 5
    if st.sidebar.button('Afficher les 5 suivants'):
        display_min += 5
        display_max += 5
    ...
    reco.rename(columns={'title' : 'Titre','year' : 'Année', 'director': 'Réalisateur', 'average_votes' : 'Note', 'genre 1' : 'Genre', 'duration' : 'Durée'}, inplace=True)
    reco['Note'] = reco['Note'].apply(lambda x : round(x))
    parsed_list, poster1, poster2, poster3, poster4, poster5 = list_parser(list(reco['imdb_title_id'].unique()[display_min:display_max]))
    ...
    st.write(reco.iloc[display_min:display_max,:5].style.set_properties(**{'background-color': 'black','color': 'red', 'font-weight' : 'bold','border-color': 'red'}).set_table_styles([{'selector': 'th', 'props': [('color', 'red')]}]))
    """
                    | Titre (Année)                                 |           Durée 	|
                    |: --------------------------------------------	|----------------:	|
                    | Réalisateur                                	| Note moyenne :  	|
                    | Acteurs Principaux                         	| 7.0             	|
                    | Description                                	|                 	|
    """
    ...
    st.markdown('# ' + str(display_min+1) + '/' + str(reco.iloc[display_min,1])+ '(' + str(reco.iloc[display_min,2])+')')
    st.image(poster1, width = 126)
    st.write('Réalisateur : ' + str(reco.iloc[display_min,9]))
    st.write('Durée : ' + str(reco.iloc[display_min,4]) + 'min')

    st.markdown('# ' + str(display_min+2) + '/' + str(reco.iloc[display_min+1,1])+ '(' + str(reco.iloc[display_min+1,2])+')')
    st.image(poster2, width = 126)
    st.write('Réalisateur : ' + str(reco.iloc[display_min+1,9]))
    st.write('Durée : ' + str(reco.iloc[display_min+1,4]) + 'min')

    st.markdown('# ' + str(display_min+3) + '/' + str(reco.iloc[display_min+2,1])+ '(' + str(reco.iloc[display_min+2,2])+')')
    st.image(poster3, width = 126)
    st.write('Réalisateur : ' + str(reco.iloc[display_min+2,9]))
    st.write('Durée : ' + str(reco.iloc[display_min+2,4]) + 'min')

    st.markdown('# ' + str(display_min+4) + '/' + str(reco.iloc[display_min+3,1])+ '(' + str(reco.iloc[display_min+3,2])+')')
    st.image(poster4, width = 126)
    st.write('Réalisateur : ' + str(reco.iloc[display_min+3,9]))
    st.write('Durée : ' + str(reco.iloc[display_min+3,4]) + 'min')

    st.markdown('# ' + str(display_min+5) + '/' + str(reco.iloc[display_min+4,1])+ '(' + str(reco.iloc[display_min+4,2])+')')
    st.image(poster5, width = 126)
    st.write('Réalisateur : ' + str(reco.iloc[display_min+4,9]))
    st.write('Durée : ' + str(reco.iloc[display_min+4,4]) + 'min')
...
# Pie chart de la répartition des films par genre
    ...
    fig = px.pie(movies_genre, values='Nombre de films', names='Genre', title='Répartition du nombre de films par genre', color_discrete_sequence=px.colors.sequential.Reds_r)
...
# Var chart sur l'évolution de la production cinématographique annuelle 
    ...
    fig.update_layout(title={'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor': 'top'}, yaxis_title="nombre de films", xaxis_title="")
    st.plotly_chart(fig, use_container_width=True)   
...
# TOP 3 des films du même genre
    ...
    Top_genre = df_movies[df_movies['genre 1']==mov_genre1][['imdb_title_id','title', 'director',  'year', 'average_votes']].sort_values(by='average_votes', ascending=False).head(3).reset_index(drop=True)
    ...
    st.image([poster1,poster2,poster3], width = 210)
    st.dataframe(Top_genre.drop(['imdb_title_id', 'Note'], axis = 1).style.set_properties(**{'background-color': 'black','color': 'red', 'font-weight' : 'bold','border-color': 'red'}).set_table_styles([{'selector': 'th', 'props': [('color', 'red')]}]))
...
# TOP 3 des films de la même année que le film sélectionné
    ...
    Top_year = df_movies[df_movies['year']==mov_year][['imdb_title_id', 'title', 'director', 'average_votes', 'genre 1']].sort_values(by='average_votes', ascending=False).head(3).reset_index(drop=True)
    Top_year.index = Top_year.index + 1
    Top_year.rename(columns={'title' : 'Titre', 'director': 'Réalisateur', 'average_votes' : 'Note', 'genre 1' : 'Genre'}, inplace=True)
    ...
    st.image([poster1,poster2,poster3], width = 210)
    st.dataframe(Top_year.drop(['imdb_title_id', 'Note'], axis = 1).style.set_properties(**{'background-color': 'black','color': 'red', 'font-weight' : 'bold','border-color': 'red'}).set_table_styles([{'selector': 'th', 'props': [('color', 'red')]}]))
